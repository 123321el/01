...
def test(net_path, text_list, ent1_list, ent2_list, result, show_result=False):
    ...
    print("correct:", correct, " ", "total:", total, " ", "准确率：（correct/total）：", correct / total)
    return rel_list


def demo_output(eval_file, data_num):  # demo_output（输入预测数据来源，数据数目）：对数据做预测，输出原文，实体对以及预测的关系，即实例输出
    text_list = []
    ent1 = []
    ent2 = []
    result = []
    total_num = data_num  # 选择预测的数据量
    with open(eval_file, 'r', encoding='utf-8') as load_f:  # 选择测试集的数据做预测
